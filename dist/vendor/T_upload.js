...
$(document).on('change', '#fileImage', function () {
    ...
    if (num < $.Tupload.fileNum + 1) {
        ...
        $('.page1-alert-small-danger').show(300).find('.page1-alert-small-danger-con').html("&nbsp;&nbsp;单次只能上传" + $.Tupload.fileNum + "张图片");
        setTimeout(function () {
...
$(document).on('click', '.onDelPic', function () {
    ...
    $("#uploadTure_" + haha).hide();
    $("#img_src" + haha).attr("value", "");
    $("#img_src" + haha).attr('name', "");
    var num = parseInt($("#fileNum").val()) - 1;
    $("#fileNum").val(num);
    arrFile[haha] = '';
    $("#img_src" + haha).attr("src", "images/imgadd.png");
    $("#uploading-tip" + haha).hide();
    $.Tupload.setPhotoImg();
});
